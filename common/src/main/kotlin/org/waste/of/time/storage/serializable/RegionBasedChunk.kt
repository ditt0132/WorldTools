...
import org.waste.of.time.mixin.SerializedChunkMixin
...
open class RegionBasedChunk(
    ...
    /**
     * See [net.minecraft.world.SerializedChunk.serialize]
     */
    ...
    private fun generateSections(chunk: WorldChunk) = NbtList().apply {
        val biomeRegistry = chunk.world.registryManager.getOptional(RegistryKeys.BIOME).orElseThrow()
    ...
    private fun NbtCompound.getTickSchedulers(chunk: WorldChunk) {
        val tickSchedulers = chunk.getTickSchedulers(chunk.world.levelProperties.time)
    ...
    private fun NbtCompound.genPostProcessing(chunk: WorldChunk) {
        put("PostProcessing", SerializedChunkMixin.toNbt(chunk.postProcessingLists))
